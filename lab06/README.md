# 🌌 Zafkiel IoT Solutions - Corporate Landing Page
...
The goal of this platform is not just to display products, but to immerse the user in the ecosystem's data flow, highlighting the seamless operation from sensor data collection to blockchain-backed rewards.
...
- **Enterprise-Grade Footer:** A 4-column responsive footer equipped with brand bio, rapid navigation links (with smooth CSS hover animations), contact information, and an integrated newsletter subscription form.
- **Interactive Elements:** Features a global Bootstrap Booking Modal (`#demoModal`) triggered across multiple call-to-action buttons for frictionless user conversion.
- **Custom CSS Animations:** Implements lightweight, inline CSS keyframes (`pulse-flow`, `bounce`, `spin-slow`) to simulate continuous data flow without relying on heavy JavaScript libraries.
...
1. **Clone or Download:** Extract the `lab06` folder to your local machine.
2. **Launch the Project:**
   - **Method A (Recommended):** Open the folder in VS Code, right-click `index.html`, and select **"Open with Live Server"**.
   - **Method B:** Simply double-click the `index.html` file to open it directly in any modern web browser (Chrome, Edge, Firefox, Safari).
